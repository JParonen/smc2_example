    ...
    for t=1:T % go through every timestep until T      
        ...
        for m = 1:NT %for each theta-particle (m)
            if t==1
                ...
                % normalization)
                temp=exp(Xweights(t-1,1:NX(t,run),m)-max(Xweights(t-1,1:NX(t,run),m)));
                temp=temp/sum(temp);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if addParticles
            tempZ=zeros(NT,t);
                for m=1:NT
                    [Z_,X_,Xw_,~]=PF_call(y(1:t),thetaParticles(:,m,t+1,run),NX(t+1,run),1);
