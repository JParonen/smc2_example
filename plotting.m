...
ylabel('std of marginal log-lik','Interpreter','latex')
title('Std of $\log(p_{\theta}(y_{1:t}))$','Interpreter','latex')
%% plot estimate of model evidence

mm=mean(cumsum(log(L)),2);
for i=1:size(L,2)
    plot(cumsum(log(L(:,i)))-mm);hold on
end
xlabel('Time','Interpreter','latex');
ylabel('$\log(p(y_{1:t}))-C$','Interpreter','latex')
title('Model log-evidence minus mean over all runs','Interpreter','latex')